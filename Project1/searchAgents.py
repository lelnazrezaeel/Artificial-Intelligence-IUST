...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        reachedCorners = state['reachedCorners']
        posState = state['state']
        isCornerState = posState in self.corners

        if isCornerState and posState not in reachedCorners:
            reachedCorners.append(posState)

        return isCornerState and len(reachedCorners) >= 4
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                successors.append(({
                    'state': (nextx, nexty),
                    'reachedCorners': list(reachedCorners)
                },action, 1))
        self._expanded += 1 # DO NOT CHANGE
        return successors
...
def cornersHeuristic(state, problem):
    ...
    for f in corners:
        ...
        howHardIsPath = (min(horizentalsWall) + min(verticalsWall))  
        h = direct_distance + (math.factorial(len(corners))) * howHardIsPath 
        if minimumH > h:
            minimumH = h 
    return minimumH
...
def foodHeuristic(state, problem):
    ...
    for f in food_table:
        ...
        if max(horizentalsFood) + max(verticalsFood) == 0:
            ...
            howHardIsPath = (min(horizentalsWall) + min(verticalsWall)) / (max(horizentalsFood) + max(verticalsFood))
        h = direct_distance + (math.factorial(foodGrid.count())) * howHardIsPath 
        if minimumH > h:
            minimumH = h 
    return minimumH
